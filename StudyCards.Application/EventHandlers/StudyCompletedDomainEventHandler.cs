...
public class StudyCompletedDomainEventHandler(ILogger<StudyCompletedDomainEventHandler> logger, ICQRSDispatcher dispatcher) : IDomainEventHandler<StudyCompletedDomainEvent>
{
    public async Task Handle(StudyCompletedDomainEvent domainEvent, CancellationToken cancellationToken)
    {
        ...
        if (result.IsSuccess)
            logger.LogInformation("Study completed for deck {DeckName} with {CardStudyCount} cards studied. User: {UserId}", domainEvent.DeckName, domainEvent.CardStudyCount, domainEvent.UserId);
